...
def main():
    ...
    try:
        ...
        if args.command == 'update_knowledge': 
            ...
            # --- START FIX ---
            # REMOVED all the manual model loading logic from main.py.
            # The evaluator.evaluate_hybrid_system() function is now
            # responsible for loading all its own models (see Fix 5).
            # We just need to trigger the lazy-load of the components
            # that the evaluator *needs* to pass to the HybridPredictor.
            
            try:
                logger.info("\nTriggering lazy-load of engines...")
                _ = evaluator.prolog_engine
                _ = evaluator.kg_engine
                logger.info("Engines loaded (or cached).")
            except Exception as e:
                logger.error(f"Failed during engine initialization: {e}", exc_info=True)
            # --- END FIX ---
